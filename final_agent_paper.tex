...
\noindent\textbf{LLM:} \texttt{Qwen/Qwen3-30B-A3B-Instruct-2507} via Tinker (same model ID across all LLM conditions), temperature 0.2, max 1400 tokens. \\
...
\begin{itemize}[noitemsep,topsep=2pt]
  ...
  \item \textbf{Single-Shot LLM}: one optimization prompt, no profiling feedback, same model/backend.
  \item \textbf{One-Pass Profile-Guided}: one profiler summary then one rewrite attempt, no further iteration.
\end{itemize}
Using the same model/backend for all LLM conditions isolates the contribution of \emph{agent design} rather than model capability~\cite{update3}.
...
\begin{table}[h]
\centering
\caption{Aggregate GreenPyBench results from a one-trial run. RT = mean runtime speedup, RAM = mean RAM reduction (passing tasks only). VIS = mean across all 10 tasks.}
\label{tab:results}
\small
\begin{tabular}{lcccc}
\toprule
...
\textbf{Critic--Refiner (ours)} & 90  & 64.8  & 22.4 & 52.07 \\
Reference Optimized             & 100 & 82.1 & 41.6 & --- \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Per-task static rule results.} The static rule baseline still performs strongly on memoization and string concatenation, but a measured regression appears on Duplicate Detection (negative runtime gain). This confirms that rule-only optimization is brittle across diverse workloads.

\begin{table}[h]
\centering
\caption{Per-task results. Static rule and agent columns are from the same one-trial evaluation run.}
\label{tab:pertask}
\small
\begin{tabular}{llccc}
\toprule
...
\midrule
1  & Duplicate Detection   & -5.60 & 0.00  & 69.95 \\
2  & Linear Search         & 0.0   & 0.00  & 68.04 \\
3  & Loop Aggregation      & 62.86 & 44.00 & 21.10 \\
4  & Top-K Selection       & 0.0   & 0.00  & 45.47 \\
5  & Nested-Loop Join      & 0.0   & 0.00  & 68.07 \\
6  & String Concatenation  & 98.57 & 69.19 & 67.77 \\
7  & Redundant List        & 0.0   & 0.00  & 33.49 \\
8  & Memoization           & 100.0 & 70.00 & 77.28 \\
9  & Data Structure Choice & 0.0   & 0.00  & 69.57 \\
10 & Multi-Pass Reduction  & 0.0   & 0.00  & 0.00 \\
\bottomrule
\end{tabular}
\end{table}
...
The static rule fires \texttt{heapq.nlargest} but the VIS guard rejects it: at $n=10^5$, $k=10$, heap overhead does not yet dominate sort cost. Heap selection outperforms full sort only when $k \ll n$ at large $n$. The one-trial agent run produced a passing Top-K rewrite with positive VIS (45.47), supporting the value of profiler-grounded iterative refinement on this task.
...
The structured JSON Critic prompt is the most impactful design choice. Separating diagnosis from generation reduces the rate of invalid outputs compared to early single-prompt experiments. The two-measurement rollback guard eliminated false rejections caused by cold-cache profiling variance~\cite{update2}. Using the same model/backend across all LLM conditions ensures that any observed improvement over Single-Shot reflects the iterative loop, not a model advantage~\cite{update3}.

\paragraph{What does not work.}
Hard tasks (Nested-Loop Join, Multi-Pass Reduction) require non-obvious algebraic transformations that the Refiner may fail to derive in one iteration. The one-trial results still show failures on difficult tasks, indicating that additional iterations and multi-trial aggregation remain important for stable conclusions.
...
The Green-Code Refactoring Agent demonstrates that a Critic--Refiner ReAct loop with structured JSON prompting, profiling-grounded diagnosis, and rollback-controlled acceptance provides a principled approach to automated Python efficiency optimization. In one-trial evaluation, the agent outperforms other LLM baselines in overall VIS and mean runtime speedup while matching one-pass correctness (90\%). Future work should expand to multi-trial runs for lower-variance reporting.
...
\paragraph{Changelog.}
Final version adds confirmed one-trial LLM and agent results in Tables~\ref{tab:results}--\ref{tab:pertask}, expands the two-measurement rollback guard discussion from Project Update 2~\cite{update2}, incorporates evaluation fairness clarifications from Project Update 3~\cite{update3}, and updates the implementation to support Tinker-compatible LLM endpoints.
